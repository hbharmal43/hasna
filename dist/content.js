...
function autofillWorkday(userData) {
...
function autofillRouter(userData) {
    ...
    if (hostname.includes("workday") || pathname.includes("workday")) {
        ...
        try {
            (0, workday_1.autofillWorkday)(userData);
        }
        catch (error) {
...
const waitForFormCompletion = async () => {
    return new Promise((resolve) => {
        let isTyping = false;
        let typingTimer;
...
const fillFormFields = async () => {
    try {
        ...
        const requiredFields = Array.from(formFields).filter(field => isElementVisible(field));
        if (requiredFields.length === 0) {
...
const findNextJob = () => {
    ...
    if (jobCards.length === 0) {
        ...
        const visibleListItems = allListItems.filter(li => isElementVisible(li));
...
const handleButtonClick = async (jobTitle, companyName, jobElement) => {
...
const handleSaveApplicationPopup = async () => {
    ...
    if (discardButton && isElementVisible(discardButton)) {
        discardButton.click();
        await sleep(500);
        return true;
    }
    ...
    try {
        while (isRunning) {
            ...
            try {
                ...
                await sleep(1000); // Reduced from 2000ms
                if (!await clickElement(types_1.SELECTORS.EASY_APPLY_BUTTON)) {
        ...
        switch (message.type) {
            ...
            case 'AUTOFILL_CURRENT_PAGE':
                ...
                try {
                    ...
                    (0, autofillEngine_1.autofillRouter)(message.data);
